## Complete exercise
(для выполнения этих заданий не забудьте создать index.html и подключить скрипт)
### NORMAL level
...
(Подсказка: чтобы превратить слово в 1 используйте Number(!!word), а затем посчитайте сумму)
...
(Подсказка, тут нужно использовать математические операции +-)
...
(Подсказка: чтобы получить вторую букву в слове нужно ... Загуглить How to get char from string js и поискать ответы что не используют метод charAt)
